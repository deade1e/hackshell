...
pub enum HackshellError {
    ...
    CommandNotFound,
    TaskNotFound,
    /// Task is protected and cannot be terminated via the task command
    TaskIsProtected,
    /// Explicit exit error generated by command
    Exit,
    /// Ctrl-C mainly
    Interrupted,
    /// Ctrl-D
...
impl Display for HackshellError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TaskIsProtected => write!(f, "Task is protected"),
            ...
            #[cfg(feature = "async")]
            Self::AsyncCommandInSyncContext => write!(
                f,
                "Async commands cannot be run from sync context, use run_async() instead"
            ),
        }
    }
}
